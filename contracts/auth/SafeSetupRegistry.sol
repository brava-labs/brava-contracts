...
contract SafeSetupRegistry is Multicall, Roles, ISafeSetupRegistry {
    ...
    function updateCurrentConfig(
        ...
        // Modules array entries must be valid contract addresses and unique
        for (uint256 i = 0; i < _modules.length; i++) {
            address module = _modules[i];
            require(module != address(0) && module.code.length > 0, Errors.InvalidInput("SafeSetupRegistry", "module"));
            for (uint256 j = 0; j < i; j++) {
                require(_modules[j] != module, Errors.InvalidInput("SafeSetupRegistry", "duplicateModule"));
            }
        }
