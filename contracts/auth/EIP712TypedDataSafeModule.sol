...
contract EIP712TypedDataSafeModule {
    ...
    string private constant CHAIN_SEQUENCE_TYPE = "ChainSequence(uint256 chainId,uint256 sequenceNonce,bool deploySafe,Sequence sequence)ActionDefinition(string protocolName,uint8 actionType)Sequence(string name,ActionDefinition[] actions,bytes4[] actionIds,bytes[] callData)";
    string private constant BUNDLE_TYPE = "Bundle(uint256 expiry,ChainSequence[] sequences)ActionDefinition(string protocolName,uint8 actionType)ChainSequence(uint256 chainId,uint256 sequenceNonce,bool deploySafe,Sequence sequence)Sequence(string name,ActionDefinition[] actions,bytes4[] actionIds,bytes[] callData)";
    ...
    function hashBundleForSigning(address _safeAddr, Bundle memory bundle) public view returns (bytes32) {
        bytes32 domainSeparator = keccak256(abi.encode(
            ...
            keccak256(bytes(domainVersion)),
            1, // Hardcoded chainID 1 for cross-chain compatibility
            _safeAddr, // ✅ Use Safe address as verifying contract
            keccak256("BravaSafe") // Updated salt
        ));
