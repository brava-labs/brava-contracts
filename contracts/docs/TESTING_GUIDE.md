...
- Hardhat config: `packages/contracts/hardhat.config.ts` (forking uses
  the explicit per-chain RPC for the selected fork network:
  `RPC_URL_ETHEREUM`, `RPC_URL_BASE`, or `RPC_URL_ARBITRUM`. There is no
  fallback to `NEXT_PUBLIC_RPC_URL_*` or `RPC_URL_EVM`; missing explicit RPCs
  should fail fast. Fork block is pinned by default.
