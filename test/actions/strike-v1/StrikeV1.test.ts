...
describe('Strike tests', () => {
  ...
  before(async () => {
    ...
    sUSDC = await getTypedContract<CTokenInterface>(
      'CTokenInterface',
      tokenConfig.STRIKE_V1_USDC.address
    );
    sUSDT = await getTypedContract<CTokenInterface>(
      'CTokenInterface',
      tokenConfig.STRIKE_V1_USDT.address
    );
