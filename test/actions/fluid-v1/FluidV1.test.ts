...
describe('FluidV1 tests', () => {
  ...
  const FLUID_GHO_ADDRESS = tokenConfig.FLUID_V1_GHO.address;
  ...
  const testCases: Array<{
    ...
    {
      poolAddress: tokenConfig.FLUID_V1_GHO.address,
      fToken: () => fGHO,
      poolName: 'GHO',
      underlying: 'GHO',
    },
  ...
  before(async () => {
    ...
    GHO = await getTokenContract('GHO');
    ...
    fUSDC = await getTypedContract<IFluidLending>('IFluidLending', FLUID_USDC_ADDRESS);
    fUSDT = await getTypedContract<IFluidLending>('IFluidLending', FLUID_USDT_ADDRESS);
    fGHO = await getTypedContract<IFluidLending>('IFluidLending', FLUID_GHO_ADDRESS);
    ...
    await adminVault.proposePool('FluidV1', FLUID_GHO_ADDRESS);
    await adminVault.addPool('FluidV1', FLUID_USDC_ADDRESS);
    await adminVault.addPool('FluidV1', FLUID_USDT_ADDRESS);
    await adminVault.addPool('FluidV1', FLUID_GHO_ADDRESS);

    // COVERAGE FIX: Register action contracts with AdminVault using actual deployed addresses
    const fluidSupplyActionId = getBytes4(fluidSupplyAddress);
    const fluidWithdrawActionId = getBytes4(fluidWithdrawAddress);

    // Propose and add actions
    await adminVault.proposeAction(fluidSupplyActionId, fluidSupplyAddress);
    await adminVault.proposeAction(fluidWithdrawActionId, fluidWithdrawAddress);
    await adminVault.addAction(fluidSupplyActionId, fluidSupplyAddress);
    await adminVault.addAction(fluidWithdrawActionId, fluidWithdrawAddress);
  });
