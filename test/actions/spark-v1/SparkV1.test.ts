...
describe('Spark tests', () => {
  ...
  before(async () => {
    ...
    sparkDAI = await getTypedContract<IERC20>('IERC20', SPARK_DAI_ADDRESS);
    sparkUSDS = await getTypedContract<IERC20>('IERC20', SPARK_USDS_ADDRESS);
    ...
    const sparkSupplyActionId = getBytes4(sparkSupplyAddress);
    const sparkWithdrawActionId = getBytes4(sparkWithdrawAddress);

    await adminVault.proposeAction(sparkSupplyActionId, sparkSupplyAddress);
    await adminVault.proposeAction(sparkWithdrawActionId, sparkWithdrawAddress);
    await adminVault.addAction(sparkSupplyActionId, sparkSupplyAddress);
    await adminVault.addAction(sparkWithdrawActionId, sparkWithdrawAddress);
  });
