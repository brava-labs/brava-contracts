...
describe('VesperV1 tests', () => {
  ...
  let USDC: IERC20;
  let vesperSupplyContract: VesperV1Supply;
  let vesperWithdrawContract: VesperV1Withdraw;
  ...
  before(async () => {
    ...
    for (const { poolAddress } of testCases) {
      await adminVault.proposePool('VesperV1', poolAddress);
      await adminVault.addPool('VesperV1', poolAddress);
    }

    // Register action contracts with AdminVault using deployed addresses for coverage compatibility
    const vesperSupplyActionId = getBytes4(vesperSupplyAddress);
    const vesperWithdrawActionId = getBytes4(vesperWithdrawAddress);

    await adminVault.proposeAction(vesperSupplyActionId, vesperSupplyAddress);
    await adminVault.proposeAction(vesperWithdrawActionId, vesperWithdrawAddress);
    await adminVault.addAction(vesperSupplyActionId, vesperSupplyAddress);
    await adminVault.addAction(vesperWithdrawActionId, vesperWithdrawAddress);
  });
  ...
  describe('VesperV1 Withdraw', () => {
    ...
    testCases.forEach(({ token, poolAddress }) => {
      describe(`Testing ${token}`, () => {
        ...
        it('Should withdraw the maximum amount', async () => {
          ...
          // Calculate expected underlying based on price per share
          const pricePerShare = await poolContract.pricePerShare();
          const expectedUnderlying = (BigInt(initialVesperBalance) * pricePerShare) / BigInt(1e18);
    ...
    describe('General tests', () => {
      it('Should emit the correct log on withdraw', async () => {
        ...
        const pricePerShare = await poolContract.pricePerShare();
        const expectedUnderlying = (BigInt(initialBalance) * pricePerShare) / BigInt(1e18);
