...
describe('ParaswapSwap tests', () => {
  ...
  describe('Input validation', () => {
    it('should revert with invalid swap data (via Safe)', async () => {
      ...
      const invalidCallData =
        '';
    ...
    it('should revert when destination token mismatch (direct contract call)', async () => {
      ...
      try {
        ...
        await expect(paraswapSwap.executeAction(encodedParams, 0)).to.be.revertedWithCustomError(
          paraswapSwap,
          'Paraswap__TokenMismatch'
        );
      } catch (error: any) {
    ...
    it('should revert with unsupported function selector', async () => {
      ...
      const invalidCallData =
        invalidSelector +
        '';
    ...
    it('should revert with zero input amount', async () => {
      ...
      try {
        ...
        await expect(paraswapSwap.executeAction(encodedParams, 0)).to.be.reverted; // Just test for any revert, not a specific message
      } catch (error: any) {
        const errorMessage = error?.message || '';
        if (errorMessage.includes('No liquidity')) {
          expect.fail('Test failed due to liquidity issues: ' + errorMessage);
    ...
    it('should revert if swap fails on Augustus Router', async () => {
      ...
      try {
        ...
        const minimalCallData =
          ...
          tokenConfig.USDC.address.slice(2) +
          // destToken (DAI)
          '000000000000000000000000' +
          tokenConfig.DAI.address.slice(2) +
  ...
  describe('Swap functionality', () => {
    async function executeSwap(
      ...
      const tokenOutContract = await ethers.getContractAt(
        'IERC20Metadata',
      ...
      try {
        ...
        expect(finalFromBalance).to.be.lt(
          initialFromBalance,
        ...
        expect(receivedBigInt).to.be.gte(
          expectedBigInt,
          'Received amount should meet minimum expectation'
        );
    ...
    describe('Function selector tests', () => {
      it('should use SWAP_EXACT_AMOUNT_IN selector with MaverickV2', async function () {
        this.timeout(0);
        const result = await executeSwap('USDC', 'USDT', '100', 'MaverickV2');

        if (result?.skipped) {
          expect.fail(`Swap skipped due to: ${result.error}`);
        }

        expect(result!.selector!.toLowerCase()).to.equal(
          SELECTORS.SWAP_EXACT_AMOUNT_IN.toLowerCase()
        );
      ...
      it('should use MAKER_PSM selector with LitePsm', async function () {
        this.timeout(0);
        const result = await executeSwap('USDC', 'DAI', '100', 'LitePsm');
      ...
      it('should use CURVE_V1 selector with CurveV1', async function () {
        this.timeout(0);
        const result = await executeSwap('USDC', 'USDT', '100', 'CurveV1');
      ...
      it('should use UNISWAP_V3 selector with UniswapV3', async function () {
        this.timeout(0);
        const result = await executeSwap('USDC', 'DAI', '100', 'UniswapV3');
      ...
      it('should use UNISWAP_V2 selector with UniswapV2', async function () {
        this.timeout(0);
        const result = await executeSwap('USDC', 'DAI', '100', 'UniswapV2');
      ...
      it('should use CURVE_V2 selector with CurveV2', async function () {
        this.timeout(0);
        const result = await executeSwap('USDC', 'USDT', '100', 'CurveV2');
      ...
      tokenPairs.forEach(({ tokenIn, tokenOut }) => {
        it(`should swap ${tokenIn} to ${tokenOut}`, async function () {
          ...
          try {
            ...
            if (errorMessage.includes('No liquidity')) {
              expect.fail(
                `Swap from ${tokenIn} to ${tokenOut} failed due to liquidity issues: ${errorMessage}`
              );
            } else {
              throw error;
            }
