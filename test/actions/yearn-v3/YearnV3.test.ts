...
describe('YearnV3 tests', () => {
  ...
  before(async () => {
    ...
    yDAI = await getTypedContract<IYearnVaultV3>('IYearnVaultV3', YEARN_DAI_ADDRESS);
    yajnaDAI = await getTypedContract<IYearnVaultV3>('IYearnVaultV3', AJNA_DAI_ADDRESS);
    yUSDS = await getTypedContract<IYearnVaultV3>('IYearnVaultV3', USDS_ADDRESS);
    ySkyUSDS = await getTypedContract<IYearnVaultV3>('IYearnVaultV3', SKY_USDS_ADDRESS);
  ...
  describe('YearnV3 Supply', () => {
    testCases.forEach(({ token, poolAddress, yToken }) => {
      describe(`Testing ${token}`, () => {
        it('Should supply', async () => {
          ...
          const tokenContract = await getTypedContract<IERC20>(
            'IERC20',
    ...
    describe('General tests', () => {
      ...
      it('Should not confuse underlying and share tokens', async () => {
        const pool = await getTypedContract<IYearnVaultV3>('IYearnVaultV3', YEARN_DAI_ADDRESS);
