...
describe('BuyCover tests', () => {
  ...
  it('should buy cover from Nexus Mutual using ETH', async () => {
    await signer.sendTransaction({
      to: safeAddr,
      value: ethers.parseEther('5.0'), // Increased to cover 2 ETH + premium
    });

    const buyCoverArgs: ActionArgs = {
      type: 'BuyCover',
      productId: 296, // Brava Conservative Cover (confirmed valid)
      amountToInsure: '2.0', // Increased from 1.0 to 2.0 ETH
      daysToInsure: 28,
  ...
  it('should buy cover from Nexus Mutual using a stablecoin', async () => {
    const fundAmount = 12000; // 12000 USDC (enough for 10k coverage + premium)
    await fundAccountWithToken(safeAddr, 'USDC', fundAmount);

    const buyCoverArgs: ActionArgs = {
      type: 'BuyCover',
      productId: 296,
      amountToInsure: '10000.0', // Insure 10,000 USDC instead of 1 USDC
      daysToInsure: 28,
  ...
  it('should have the NFT in the safe', async () => {
    ...
    const buyCoverArgs: ActionArgs = {
      ...
      amountToInsure: '10000.0', // Increased from 1.0 to 10000.0 USDC
      daysToInsure: 28,
  ...
  it('should have the correct data in the NFT', async () => {
    ...
    expect((metadataJson as any).name).to.equal('Nexus Mutual: Cover NFT');
  ...
  it('Should emit the correct log', async () => {
    ...
    // For USDC with 6 decimals, should be 10000e6 (10000000000) for 10000.0 USDC
    expect(logs[0]).to.have.property('amount', ethers.parseUnits('10000.0', 6).toString());
