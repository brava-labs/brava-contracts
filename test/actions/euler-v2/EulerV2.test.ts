...
describe('EulerV2 tests', () => {
  ...
  before(async () => {
    ...
    ePrimeUSDC = await getTypedContract<IEulerV2Lending>(
      'IEulerV2Lending',
      tokenConfig.EULER_V2_PRIME_USDC.address
    );
    eYieldUSDC = await getTypedContract<IEulerV2Lending>(
      'IEulerV2Lending',
      tokenConfig.EULER_V2_YIELD_USDC.address
    );
    eYieldUSDT = await getTypedContract<IEulerV2Lending>(
      'IEulerV2Lending',
      tokenConfig.EULER_V2_YIELD_USDT.address
    );
    eYieldUSDE = await getTypedContract<IEulerV2Lending>(
      'IEulerV2Lending',
      tokenConfig.EULER_V2_YIELD_USDE.address
    );
    eMaxiUSDC = await getTypedContract<IEulerV2Lending>(
      'IEulerV2Lending',
      tokenConfig.EULER_V2_MAXI_USDC.address
    );
    eResolvUSDC = await getTypedContract<IEulerV2Lending>(
      'IEulerV2Lending',
      tokenConfig.EULER_V2_RESOLV_USDC.address
    );
    ...
    const eulerSupplyActionId = getBytes4(eulerSupplyAddress);
    const eulerWithdrawActionId = getBytes4(eulerWithdrawAddress);

    await adminVault.proposeAction(eulerSupplyActionId, eulerSupplyAddress);
    await adminVault.proposeAction(eulerWithdrawActionId, eulerWithdrawAddress);
    await adminVault.addAction(eulerSupplyActionId, eulerSupplyAddress);
    await adminVault.addAction(eulerWithdrawActionId, eulerWithdrawAddress);
  });
    ...
    testCases.forEach(({ poolName, poolAddress, underlying, eToken }) => {
      describe(`Testing ${poolName}`, () => {
        ...
        it('Should take fees on deposit', async () => {
          ...
          if (!firstTxReceipt) {
            throw new Error('First deposit transaction failed to get receipt');
          }

          const secondTxReceipt = await secondTx.wait();
          if (!secondTxReceipt) {
            throw new Error('Second deposit transaction failed to get receipt');
          }
    ...
    describe('General tests', () => {
      it('Should emit the correct log on deposit', async () => {
        ...
        const poolId: BytesLike = ethers
          .keccak256(tokenConfig.EULER_V2_PRIME_USDC.address)
          .slice(0, 10);
    ...
    testCases.forEach(({ poolName, poolAddress, underlying, eToken }) => {
      describe(`Testing ${poolName}`, () => {
        ...
        it('Should take fees on withdraw', async () => {
          ...
          if (!supplyReceipt) {
            throw new Error('Supply transaction failed to get receipt');
          }

          const withdrawReceipt = await withdrawTx.wait();
          if (!withdrawReceipt) {
            throw new Error('Withdraw transaction failed to get receipt');
          }
