...
describe('YearnV2 tests', () => {
  ...
  before(async () => {
    ...
    yUSDC = await getTypedContract<IYearnVault>('IYearnVault', YEARN_USDC_ADDRESS);
    yUSDT = await getTypedContract<IYearnVault>('IYearnVault', YEARN_USDT_ADDRESS);
    yDAI = await getTypedContract<IYearnVault>('IYearnVault', YEARN_DAI_ADDRESS);
    ...
    describe('General tests', () => {
      ...
      it('Should not confuse underlying and share tokens', async () => {
        ...
        const expectedUnderlying =
          BigInt(amount) - BigInt(depositAmount) + BigInt(depositAmount) / BigInt(10);
