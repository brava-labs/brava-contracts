...
import { signBundle, createBundle, validateBundleSignature, type Bundle } from '../utils-eip712';
...
describe('EIP712TypedDataSafeModule', function () {
  ...
  describe('Deployment', function () {
    it('should deploy with correct parameters', async function () {
      ...
      expect(await eip712Module.SEQUENCE_EXECUTOR_ADDR()).to.equal(
  ...
  describe('EIP-712 Signature Generation and Validation', function () {
    ...
    it('should create different signatures for different signers', async function () {
      ...
      expect(await validateBundleSignature(bundle, adminSignature, admin.address, adminSafeAddr, 1))
        .to.be.true;
      expect(await validateBundleSignature(bundle, aliceSignature, alice.address, aliceSafeAddr, 1))
        .to.be.true;

      // But not for the wrong signer
      expect(await validateBundleSignature(bundle, adminSignature, alice.address, adminSafeAddr, 1))
        .to.be.false;
    });
  });
  ...
  describe('Access Control', function () {
    it('should reject invalid signatures', async function () {
      ...
      await expect(eip712Module.executeBundle(safeAddr, bundle, invalidSignature)).to.be.reverted; // Will revert with ECDSA error for invalid signature length
    });
  });
  ...
  describe('Expiry Management', function () {
    ...
    it('should reject bundles that have expired', async function () {
      ...
      await expect(eip712Module.executeBundle(safeAddr, expiredBundle, signature)).to.be.reverted; // Should revert due to expiry
    });
  });
