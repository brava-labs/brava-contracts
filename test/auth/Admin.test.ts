...
describe('AdminVault', function () {
  ...
  describe('Indirect tests', function () {
    ...
    // let fluidSupplyAddress: string; // Unused variable
    before(async () => {
      const signers = await ethers.getSigners();
      signer = signers[0]!;
      ...
      // fluidSupplyAddress = await fluidSupplyContract.getAddress(); // Unused variable
    ...
    it('should calculate fee correctly for a given period', async function () {
      ...
      //   ethers.keccak256(
      //     ethers.AbiCoder.defaultAbiCoder().encode(["string"], ["FluidV1"]),
      //   ),
      // ); // Unused variable
