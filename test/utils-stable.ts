...
// Removed @ethersproject/bignumber import - using native BigInt with ethers v6
// HardhatRuntimeEnvironment import removed - not used in this file
import { formatUnits } from 'ethers';
import * as constants from './constants';
import { log, getTypedContract } from './utils';
...
): Promise<IERC20Metadata | Record<string, IERC20Metadata>> {
...
const getTokenContractSingle = async (tokenSymbol: string): Promise<IERC20Metadata> => {
  ...
  return getTypedContract<IERC20Metadata>('IERC20Metadata', token.address);
};
...
async function fundAccountWithToken(
  ...
  const tokenContract = await getTypedContract<IERC20>('IERC20', token.address, whaleSigner);
