...
async function testnetSetup() {
  ...
  const safeAddress = await deploySafeForSigner(
    testAccounts[0],
    await contracts.baseSetup.safeProxyFactory.getAddress()
  );

  // Approve USDC for the Safe
  await approveTokenForSafe(
    testAccounts[0],
    safeAddress,
    constants.tokenConfig.USDC.address,
    ethers.MaxUint256
  );
  ...
  await approveTokenForSafe(
    testAccounts[0],
    safeAddress,
    constants.tokenConfig.USDT.address,
    ethers.MaxUint256
  );
  await deposit(
    testAccounts[0],
