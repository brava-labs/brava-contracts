...
export async function deploySafeForSigner(
  signer: Signer,
...
export async function approveTokenForSafe(
  signer: Signer,
  safeAddress: string,
  tokenAddress: string,
  amount: bigint
  ...
  console.log(
    `Approved ${amount} ${await token.symbol()} for safe ${safeAddress} by ${await signer.getAddress()}`
  );
}
