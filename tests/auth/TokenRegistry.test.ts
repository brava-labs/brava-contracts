...
describe('TokenRegistry', () => {
  ...
  before(async () => {
    ...
    tokenRegistry = await deploy<TokenRegistry>(
      'TokenRegistry',
      deployer,
  ...
  describe('Token Management', () => {
    ...
    it('should revert when proposing zero address token', async () => {
      await expect(tokenRegistry.proposeToken(ethers.ZeroAddress))
        .to.be.revertedWithCustomError(tokenRegistry, 'InvalidInput');
    });
    ...
    it('should revert when proposing already approved token', async () => {
      ...
      await expect(tokenRegistry.proposeToken(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_TransactionAlreadyApproved');
    });
    ...
    it('should revert when canceling non-existent proposal', async () => {
      await expect(tokenRegistry.cancelTokenProposal(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_TransactionNotProposed');
    });
    ...
    it('should revert when approving token before delay period', async () => {
      ...
      await expect(tokenRegistry.approveToken(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_DelayNotPassed');
    });
    ...
    it('should revert when approving zero address token', async () => {
      await expect(tokenRegistry.approveToken(ethers.ZeroAddress))
        .to.be.revertedWithCustomError(tokenRegistry, 'InvalidInput');
    });
    ...
    it('should revert when revoking non-approved token', async () => {
      await expect(tokenRegistry.revokeToken(testToken))
        .to.be.revertedWith('TokenRegistry: Token not approved');
    });
    ...
    it('should revert operations when caller lacks required role', async () => {
      await expect(tokenRegistry.connect(user).proposeToken(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_MissingRole');

      await tokenRegistry.proposeToken(testToken);

      await expect(tokenRegistry.connect(user).cancelTokenProposal(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_MissingRole');

      await expect(tokenRegistry.connect(user).approveToken(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_MissingRole');

      await tokenRegistry.approveToken(testToken);

      await expect(tokenRegistry.connect(user).revokeToken(testToken))
        .to.be.revertedWithCustomError(tokenRegistry, 'AdminVault_MissingRole');
    });
  });
}); 