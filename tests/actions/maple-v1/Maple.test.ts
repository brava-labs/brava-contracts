...
import { AdminVault, MapleSupply, MapleWithdrawQueue, Logger, IERC20 } from '../../../typechain-types';
import { getBaseSetup, deploy, executeAction, approveMapleKYC, processMapleWithdrawal } from '../../utils';
...
describe('Maple protocol actions', () => {
  ...
  before(async () => {
    ...
    mapleSupply = await deploy('MapleSupply', signer, await adminVault.getAddress(), await logger.getAddress());
    mapleWithdrawQueue = await deploy('MapleWithdrawQueue', signer, await adminVault.getAddress(), await logger.getAddress());
  ...
  describe('MapleSupply', () => {
    for (const [key, poolConfig] of Object.entries(tokenConfig)) {
      if (!key.startsWith('MAPLE_V1_')) continue;
    ...
    describe('General tests', () => {
      it('Should emit the correct log on deposit', async () => {
        ...
        const [key, poolConfig] = Object.entries(tokenConfig).find(([k]) => k.startsWith('MAPLE_V1_'))!;
        const amount = ethers.parseUnits('2000', poolConfig.decimals);
        await fundAccountWithToken(safeAddr, 'USDC', amount);
      ...
      it('Should initialize last fee timestamp', async () => {
        // Use the first MAPLE_V1_ pool
        const [key, poolConfig] = Object.entries(tokenConfig).find(([k]) => k.startsWith('MAPLE_V1_'))!;
        const initialLastFeeTimestamp = await adminVault.lastFeeTimestamp(safeAddr, poolConfig.address);
        ...
        const finalLastFeeTimestamp = await adminVault.lastFeeTimestamp(safeAddr, poolConfig.address);
  ...
  describe('MapleWithdrawQueue', () => {
    ...
    describe('General tests', () => {
      it('Should emit the correct log on withdrawal request', async () => {
        ...
        const withdrawalManager = await ethers.getContractAt('IMapleWithdrawalManagerQueue', await poolManager.withdrawalManager());
  ...
  describe('Workflow tests', () => {
    it('should demonstrate the correct withdrawal workflow', async () => {
      // Use the first MAPLE_V1_ pool from tokenConfig
      const [ , poolConfig ] = Object.entries(tokenConfig).find(([k]) => k.startsWith('MAPLE_V1_'))!;
